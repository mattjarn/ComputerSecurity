...
public class Common {
     ...
     * @throws OTPCheatException 
     */
    public static byte[] encryptKey(RSAPublicKey K_u, SecretKey K,
                                    SecureRandom r)
      throws NoSuchAlgorithmException, InvalidKeyException,
             IllegalBlockSizeException, BadPaddingException,
             NoSuchPaddingException, OTPCheatException {

        Cipher c = Cipher.getInstance("RSA/ECB/NoPadding");
        c.init(Cipher.ENCRYPT_MODE, K_u);
        byte[] K_data = K.getEncoded();
        int block_len = (K_u.getModulus().bitLength() + 7) / 8;
        System.out.println(block_len + "\n");
        System.out.println(K_data.length + "\n");
       if(block_len !=256)
        {
        	throw new OTPCheatException("Block length not the right size");
        }
